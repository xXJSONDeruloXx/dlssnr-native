...
static void transform(nr_sha256 *ctx, const uint8_t block[64]) {
    ...
    uint32_t w[64];
    for (unsigned i=0;i<16;i++) w[i]=load32be(block+i*4);
...
void nr_sha256_final(nr_sha256 *ctx, uint8_t digest[32]) {
    ...
    for (unsigned i=0;i<8;i++) ctx->block[63-i]=(uint8_t)(bits>>(i*8));
    transform(ctx,ctx->block);
    for (unsigned i=0;i<8;i++) store32be(digest+i*4,ctx->state[i]);
